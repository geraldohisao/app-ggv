-- Script corrigido para criar tabela debug_logs
...
CREATE POLICY "debug_logs_insert_policy" ON debug_logs
  FOR INSERT 
  WITH CHECK (auth.uid() IS NOT NULL);

-- Política para permitir leitura apenas para super admins
CREATE POLICY "debug_logs_select_policy" ON debug_logs
  FOR SELECT 
  USING (
    EXISTS (
...
-- Inserir um log de teste para verificar se está funcionando
INSERT INTO debug_logs (level, source, message, user_name, user_email) 
VALUES ('info', 'System', 'Tabela debug_logs criada com sucesso', 'Sistema', 'sistema@grupoggv.com');

-- Verificar se a tabela foi criada corretamente
SELECT 'Tabela debug_logs criada com sucesso!' as status,
       COUNT(*) as total_logs
FROM debug_logs;
