-- SCRIPT DEFINITIVO PARA CORRIGIR A FUNÇÃO get_calls_with_filters
-- Usando as tabelas corretas do sistema PostgreSQL

-- 1. Listar todas as funções get_calls_with_filters existentes (usando pg_proc)
SELECT 
...
-- 2. Remover usando DROP CASCADE para todas as possíveis variações
DO $$
DECLARE
    func_record RECORD;
    drop_sql TEXT;
BEGIN
    ...
    -- Se não encontrou nenhuma função, tentar drops manuais
    IF NOT FOUND THEN
        RAISE NOTICE 'Nenhuma função encontrada, tentando drops manuais...';
        
        -- Tentar várias combinações possíveis
        BEGIN
...
SELECT 
    CASE 
        WHEN COUNT(*) = 0 THEN 'Todas as funções get_calls_with_filters foram removidas!'
...
-- 4. Criar a nova função com filtro min_score
...
BEGIN
    ...
    filtered_calls AS (
        ...
        FROM base_calls bc
        WHERE 
            -- FILTRO DE SCORE MÍNIMO - AQUI É A MÁGICA!
...
-- 6. Verificação final
SELECT 'SUCESSO: Função get_calls_with_filters criada com filtro min_score!' as status;

-- 7. Teste da função com score >= 7
SELECT 
    'TESTE SCORE >= 7' as teste,
    COUNT(*) as chamadas_encontradas,
    MIN(score) as menor_score,
