-- 🔧 CORRIGIR: Função get_calls_with_filters usando campos corretos
-- Execute este script no SQL Editor do Supabase

-- 1. Remover função atual
DROP FUNCTION IF EXISTS get_calls_with_filters CASCADE;

-- 2. Criar função corrigida usando campos diretos da tabela
...
AS $$
    SELECT 
        ...
        -- CORREÇÃO: Usar campos diretos da tabela
    ...
    WHERE 
        ...
        AND (p_search_query IS NULL OR 
             ...
             c.person ILIKE '%' || p_search_query || '%' OR
...
SELECT 'Função corrigida usando campos diretos!' as status;
