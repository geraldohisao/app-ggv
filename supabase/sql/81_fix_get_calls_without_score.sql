-- ===================================================================
-- FIX GET_CALLS WITHOUT SCORE - Corrigir função sem coluna score
-- ===================================================================

-- 1. Verificar quais colunas existem na tabela calls
SELECT column_name, data_type 
FROM information_schema.columns 
WHERE table_name = 'calls' AND table_schema = 'public'
ORDER BY ordinal_position;

-- 2. Recriar a função get_calls_with_filters SEM a coluna score
...
RETURNS TABLE (
    ...
    insights JSONB,
    scorecard JSONB,
    score INTEGER,  -- Será calculado dinamicamente
    from_number TEXT,
    to_number TEXT,
    agent_id TEXT,
...
BEGIN
    ...
    SELECT 
        ...
        fc.insights,
        fc.scorecard,
        -- Score calculado dinamicamente do scorecard se existir
        CASE 
...
SELECT 
    'Teste da função corrigida sem coluna score' as teste,
    company_name,
    person_name,
    sdr_name,
    score,
...
-- 5. Verificar se ainda há "Empresa Desconhecida"
SELECT 
    COUNT(*) as total_calls,
    COUNT(CASE WHEN company_name = 'Empresa Desconhecida' THEN 1 END) as empresas_desconhecidas,
    COUNT(CASE WHEN company_name NOT LIKE 'Empresa Desconhecida' AND company_name NOT LIKE 'Empresa %' THEN 1 END) as empresas_com_nome_real
FROM get_calls_with_filters(NULL, NULL, NULL, NULL, NULL, 100, 0);

-- 6. Mostrar exemplos de empresas encontradas
SELECT DISTINCT 
    company_name,
    COUNT(*) as quantidade
FROM get_calls_with_filters(NULL, NULL, NULL, NULL, NULL, 100, 0)
...
SELECT 'Função get_calls_with_filters corrigida sem coluna score!' as status;
