-- workspace_sync_user robusto:
-- - Compatível com frontend (8 params + manager opcional no final)
-- - Evita 23505 (profiles_google_id_key) limpando duplicatas por google_id antes do UPSERT por email
-- - Não sobrescreve foto real por avatar genérico do Google (lh3.google.com/ao/)
...
BEGIN
  ...
  SELECT EXISTS(SELECT 1 FROM profiles WHERE email = p_email) INTO existed_before;
  ...
  SET
    ...
    avatar_url = CASE
      ...
      WHEN EXCLUDED.avatar_url ~* 'lh3\.google\.com/ao/' THEN
        CASE
          WHEN profiles.avatar_url IS NULL OR profiles.avatar_url = '' OR profiles.avatar_url ~* 'lh3\.google\.com/ao/' THEN EXCLUDED.avatar_url
...
COMMENT ON FUNCTION public.workspace_sync_user IS 'Importa/atualiza usuário do Google Workspace (upsert por email + regra avatar genérico + resolve conflito google_id)';
