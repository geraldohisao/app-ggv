-- 40_calls_system_complete_fixed.sql
-- SCRIPT COMPLETO CORRIGIDO: Todas as dependências na ordem correta
-- Execute APENAS este script (substitui todos os anteriores)

-- =========================================
-- ETAPA 0: CRIAR TABELAS DE SUPORTE (SE NÃO EXISTIREM)
-- =========================================

-- Tabela de comentários de calls
...
-- Tabela de critérios dos scorecards
...
-- Tabela de scores detalhados por call
...
-- Configurar RLS para tabelas de suporte
...
DROP POLICY IF EXISTS "Users can view call comments" ON call_comments;
...
DROP POLICY IF EXISTS "Users can view scorecard criteria" ON scorecard_criteria;
CREATE POLICY "Users can view scorecard criteria" ON scorecard_criteria 
    FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Users can view call scores" ON call_scores;
CREATE POLICY "Users can view call scores" ON call_scores 
    FOR SELECT USING (auth.role() = 'authenticated');

-- Índices para performance das tabelas de suporte
...
-- =========================================
-- ETAPA 1: LIMPAR SISTEMA ANTIGO COMPLETO
-- =========================================

-- Remover TODAS as funções antigas (com todos os tipos possíveis)
...
DROP FUNCTION IF EXISTS public.extract_company_name(JSONB, TEXT, UUID);
DROP FUNCTION IF EXISTS public.extract_person_data(JSONB);

-- Remover funções de dashboard e gráficos se existirem
...
DROP FUNCTION IF EXISTS public.get_call_volume_chart(INTEGER);
...
-- =========================================
-- ETAPA 3: FUNÇÕES AUXILIARES (ORDEM CORRETA)
-- =========================================

-- 1. Função para normalizar emails SDR (SEM DEPENDÊNCIAS)
...
-- 2. Função para mapear nome do SDR (SEM DEPENDÊNCIAS)
...
-- 3. Função para extrair dados da empresa do insights (SEM DEPENDÊNCIAS)
...
-- 4. Função para extrair dados da pessoa do insights (SEM DEPENDÊNCIAS)
...
-- Gráfico de scores por SDR (CORRIGIDO - SEM AMBIGUIDADE)
...
SELECT 'SDR Score Chart Test:' as test, COUNT(*) as sdr_count FROM get_sdr_score_chart(14);
...
-- Verificar tabelas de suporte criadas
SELECT 
  'Support Tables:' as info,
  (SELECT COUNT(*) FROM call_comments) as comments_count,
  (SELECT COUNT(*) FROM scorecards) as scorecards_count,
  (SELECT COUNT(*) FROM scorecard_criteria) as criteria_count,
  (SELECT COUNT(*) FROM call_scores) as scores_count;
