-- 32_complete_calls_system.sql
-- Sistema completo de calls com análise IA, comentários, scores e scorecards
...
-- Função para buscar análise completa de uma call
...
AS $$
    SELECT 
        ...
        COALESCE(
            ...
            LEFT JOIN scorecard_criteria sc ON s.id = sc.scorecard_id
            WHERE s.call_type = c.call_type AND s.is_active = true
            GROUP BY s.id, s.name, s.description),
            '{}'::jsonb
        ) as scorecard_data
    ...
    GROUP BY c.id, c.insights, c.deal_id, c.duration, c.recording_url, 
             c.transcription, c.status, c.created_at, c.scorecard, c.call_type;
$$;
