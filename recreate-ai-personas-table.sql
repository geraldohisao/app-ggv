-- Script para RECRIAR COMPLETAMENTE a tabela ai_personas
-- Execute este script no SQL Editor do seu projeto Supabase

-- 1. Verificar se a tabela existe
SELECT 'Verificando tabela ai_personas...' as status;

-- 2. REMOVER a tabela existente (se existir)
DROP TABLE IF EXISTS ai_personas CASCADE;

-- 3. CRIAR a tabela do zero com todas as colunas corretas
...
-- 6. INSERIR as personas com todas as configurações corretas
...
SELECT 
    ...
    wordLimit,
    CASE 
        WHEN LENGTH(systemPrompt) > 50 THEN LEFT(systemPrompt, 50) || '...'
        ELSE systemPrompt
    END as systemPrompt_preview,
    CASE 
        WHEN LENGTH(directives) > 50 THEN LEFT(directives, 50) || '...'
        ELSE directives
    END as directives_preview,
    personalityTraits
FROM ai_personas;

-- 9. VERIFICAÇÃO FINAL
SELECT 
    'CRIAÇÃO CONCLUÍDA:' as status,
    COUNT(*) as total_personas,
    COUNT(CASE WHEN personalityTraits IS NOT NULL THEN 1 END) as com_traits,
    COUNT(CASE WHEN systemPrompt IS NOT NULL AND systemPrompt != '' THEN 1 END) as com_prompts,
    COUNT(CASE WHEN directives IS NOT NULL AND directives != '' THEN 1 END) as com_directives,
    '✅ TABELA RECRIADA COM SUCESSO' as resultado
FROM ai_personas;
