-- 🤖 SISTEMA DE ANÁLISE AUTOMÁTICA - VERSÃO FINAL
-- Sem ON CONFLICT, usando WHERE NOT EXISTS

-- ===============================================================
-- ETAPA 1: VERIFICAR E AJUSTAR TABELA analysis_queue
-- ===============================================================
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'analysis_queue_call_id_unique'
    ) THEN
        ALTER TABLE analysis_queue 
        ADD CONSTRAINT analysis_queue_call_id_unique UNIQUE (call_id);
    END IF;
END $$;

-- ===============================================================
-- ETAPA 2: TRIGGER AUTOMÁTICO (SEM ON CONFLICT)
-- ===============================================================
...
BEGIN
    ...
    IF (NEW.duration >= 180 OR 
       ...
       AND NOT EXISTS (
           SELECT 1 FROM analysis_queue WHERE call_id = NEW.id
       ) THEN
...
-- ===============================================================
-- ETAPA 4: FUNÇÃO PARA BUSCAR PRÓXIMO LOTE
-- ===============================================================
...
-- ===============================================================
-- ETAPA 5: POPULAR FILA COM CHAMADAS EXISTENTES
-- ===============================================================

-- Adicionar chamadas que ainda não foram analisadas
INSERT INTO analysis_queue (call_id, priority, created_at)
SELECT 
    c.id,
    CASE 
...
-- ===============================================================
-- ETAPA 6: VERIFICAÇÃO DO SISTEMA
-- ===============================================================
...
-- ===============================================================
-- ETAPA 7: COMANDOS ÚTEIS PARA MONITORAMENTO
-- ===============================================================

-- Ver estatísticas da fila
SELECT 
    'ESTATÍSTICAS DA FILA' as info,
    status,
    priority,
    CASE priority 
        WHEN 1 THEN 'urgent'
