-- CORRIGIR MAPEAMENTO DE EMPRESA E PESSOA NA FUNÇÃO
...
SELECT 
    ...
    fc.provider_call_id,
    fc.deal_id,
    -- Mapear empresa (tentar diferentes campos do insights)
    COALESCE(
        fc.insights->>'company', 
        fc.insights->>'enterprise',
        fc.insights->>'company_name',
        fc.insights->>'organization',
        'Empresa não informada'
    ) as company_name,
    -- Mapear pessoa (tentar diferentes campos do insights)
    COALESCE(
        fc.insights->>'person', 
        fc.insights->>'person_name',
        fc.insights->>'contact_name',
        fc.insights->>'name'
    ) as person_name,
    fc.insights->>'person_email' as person_email,
    fc.agent_id as sdr_id,
    fc.agent_id as sdr_name,
    fc.agent_id as sdr_email,
    CONCAT('https://i.pravatar.cc/64?u=', COALESCE(fc.agent_id, 'default')) as sdr_avatar_url,
    fc.status,
    fc.status_voip,
    ...
    fc.audio_bucket,
    fc.audio_path,
    COALESCE(fc.recording_url, CONCAT(COALESCE(fc.audio_bucket, ''), '/', COALESCE(fc.audio_path, ''))) as audio_url,
    fc.transcription,
