-- add-followup-scorecard-safe.sql
-- Versão segura que verifica a estrutura da tabela antes de inserir
...
-- Verificar se a tabela scorecards existe e suas colunas
DO $$
DECLARE
...
BEGIN
    ...
    IF NOT table_exists THEN
        RAISE EXCEPTION 'Tabela scorecards não existe. Execute primeiro o script 29_create_calls_support_tables.sql';
    END IF;
    ...
    IF has_is_active THEN
        RAISE NOTICE 'Coluna is_active encontrada. Usando estrutura completa.';
    ELSE
        RAISE NOTICE 'Coluna is_active não encontrada. Usando estrutura básica.';
    END IF;
END $$;
...
-- Tentar inserir com is_active se a coluna existir
DO $$
DECLARE
    has_is_active boolean;
BEGIN
    ...
    RAISE NOTICE 'Scorecard Follow Up inserido com sucesso!';
EXCEPTION
    WHEN unique_violation THEN
        RAISE NOTICE 'Scorecard Follow Up já existe. Continuando...';
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Erro ao inserir scorecard: %', SQLERRM;
END $$;
...
ON CONFLICT (scorecard_id, order_index) DO UPDATE SET
    text = EXCLUDED.text,
