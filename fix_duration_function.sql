-- CORRIGIR FUNÇÃO PARA BUSCAR CALLS COM DURATION_FORMATTED (SEM AMBIGUIDADE)
...
WITH filtered_calls AS (
    SELECT 
        ...
        -- Campos derivados com aliases únicos
        COALESCE(c.insights->>'company', c.insights->>'enterprise', 'Empresa não informada') as derived_company_name,
        COALESCE(c.insights->>'person', c.insights->>'person_name') as derived_person_name,
        c.insights->>'person_email' as derived_person_email,
        c.agent_id as derived_sdr_id,
        c.agent_id as derived_sdr_name,
        c.agent_id as derived_sdr_email,
        CONCAT('https://i.pravatar.cc/64?u=', COALESCE(c.agent_id, 'default')) as derived_sdr_avatar_url,
        COALESCE(c.recording_url, CONCAT(COALESCE(c.audio_bucket, ''), '/', COALESCE(c.audio_path, ''))) as derived_audio_url
    FROM calls c
    WHERE 
...
SELECT 
    ...
    fc.provider_call_id,
    fc.deal_id,
    fc.derived_company_name as company_name,
    fc.derived_person_name as person_name,
    fc.derived_person_email as person_email,
    fc.derived_sdr_id as sdr_id,
    fc.derived_sdr_name as sdr_name,
    fc.derived_sdr_email as sdr_email,
    fc.derived_sdr_avatar_url as sdr_avatar_url,
    fc.status,
    fc.status_voip,
    ...
    fc.audio_bucket,
    fc.audio_path,
    fc.derived_audio_url as audio_url,
    fc.transcription,
