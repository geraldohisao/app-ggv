-- 🔧 CORRIGIR: Tradução de status na RPC get_calls_with_filters
-- Execute este script no SQL Editor do Supabase

-- 1. Remover função atual
DROP FUNCTION IF EXISTS get_calls_with_filters CASCADE;

-- 2. Criar função corrigida com tradução de status
...
AS $$
  SELECT
    ...
    c.to_number,
    c.from_number,
    -- ✅ NOTA: pegar de call_analysis.final_grade se existir; senão dos campos do scorecard
    COALESCE(
      ca.final_grade,
      NULLIF(c.scorecard->>'final_score','')::NUMERIC,
      NULLIF(c.scorecard->>'total_score','')::NUMERIC,
      NULLIF(c.scorecard->>'score','')::NUMERIC
    ) AS score,
...
SELECT 'Função corrigida com tradução de status!' as status;
