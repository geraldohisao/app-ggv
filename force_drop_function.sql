-- FORÇAR REMOÇÃO DE TODAS AS VERSÕES DA FUNÇÃO get_calls_with_filters
-- Este script remove todas as possíveis variações da função

-- 1. Listar todas as funções get_calls_with_filters existentes
SELECT 
    schemaname,
    functionname,
    arguments
FROM pg_functions 
WHERE functionname = 'get_calls_with_filters';

-- 2. Remover todas as possíveis versões da função (diferentes assinaturas)
DROP FUNCTION IF EXISTS public.get_calls_with_filters() CASCADE;
DROP FUNCTION IF EXISTS public.get_calls_with_filters(TEXT) CASCADE;
DROP FUNCTION IF EXISTS public.get_calls_with_filters(TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS public.get_calls_with_filters(TEXT, TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS public.get_calls_with_filters(TEXT, TEXT, TEXT, TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS public.get_calls_with_filters(TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER) CASCADE;
DROP FUNCTION IF EXISTS public.get_calls_with_filters(TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, INTEGER) CASCADE;
DROP FUNCTION IF EXISTS public.get_calls_with_filters(TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, INTEGER, TEXT) CASCADE;
DROP FUNCTION IF EXISTS public.get_calls_with_filters(TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, INTEGER, TEXT, INTEGER, INTEGER) CASCADE;
DROP FUNCTION IF EXISTS public.get_calls_with_filters(TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, INTEGER, TEXT, INTEGER, INTEGER, NUMERIC) CASCADE;

-- 3. Remover usando DROP CASCADE para todas as possíveis variações
DO $$
DECLARE
    func_record RECORD;
BEGIN
    -- Buscar todas as funções com nome get_calls_with_filters
    FOR func_record IN 
        SELECT 
    ...
    LOOP
        -- Executar DROP para cada função encontrada
        EXECUTE format('DROP FUNCTION IF EXISTS %I.%I(%s) CASCADE', 
                      func_record.schema_name, 
                      func_record.function_name, 
                      func_record.arguments);
        
        RAISE NOTICE 'Removida função: %.%(%)', 
                     func_record.schema_name, 
                     func_record.function_name, 
                     func_record.arguments;
    END LOOP;
...
SELECT 
    CASE 
        WHEN COUNT(*) = 0 THEN 'Todas as funções get_calls_with_filters foram removidas com sucesso!'
        ELSE 'Ainda existem ' || COUNT(*) || ' funções get_calls_with_filters'
    END as status
FROM pg_functions 
WHERE functionname = 'get_calls_with_filters';

-- 5. Agora criar a nova função
...
-- 7. Verificação final
SELECT 'Função get_calls_with_filters criada com sucesso com filtro min_score!' as status;

-- 8. Teste rápido da função
SELECT 
    'Testando função' as teste,
