-- Corrigir função get_calls_with_filters para incluir chamadas sem agent_id
-- O problema é que chamadas longas não têm agent_id e estão sendo filtradas

-- 1. Primeiro, vamos ver a função atual
SELECT 'Verificando função get_calls_with_filters atual...' as status;

-- 2. Recriar a função sem filtro obrigatório de agent_id
...
BEGIN
  ...
  WHERE 
    ...
    -- REMOVIDO: AND fc.agent_id IS NOT NULL (isso estava excluindo chamadas longas!)
...
-- Verificar se agora retorna chamadas longas
SELECT 
...
-- Mostrar algumas chamadas longas que agora devem aparecer
SELECT 
  id,
  duration,
  duration_formated,
...
SELECT 'Função corrigida! Chamadas longas devem aparecer agora.' as resultado;
