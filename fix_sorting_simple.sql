-- ===================================================================
-- FIX SORTING SIMPLE - Corrigir ordenação de forma simples
-- ===================================================================

-- 1. Testar se a função get_calls_with_sorting existe
SELECT 
    COUNT(*) as funcao_existe
FROM pg_proc 
WHERE proname = 'get_calls_with_sorting';

-- 2. Se não existir, vamos modificar a função original
...
BEGIN
    ...
    WITH filtered_calls AS (
        ...
        ORDER BY 
            CASE 
                WHEN p_sort_by = 'duration' THEN c.duration
                ELSE NULL
            END DESC NULLS LAST,
            CASE 
                WHEN p_sort_by = 'score' THEN 
                    COALESCE((c.scorecard->>'overall_score')::INTEGER, 
                            (c.scorecard->>'score')::INTEGER, 0)
                ELSE NULL
            END DESC NULLS LAST,
            CASE 
                WHEN p_sort_by = 'company' THEN 
                    COALESCE(c.enterprise, c.insights->>'companyName', c.insights->>'company')
                ELSE NULL
            END ASC NULLS LAST,
            c.created_at DESC -- Ordenação padrão
...
-- 3. Testar função corrigida ordenando por duração
SELECT 
    'TESTE ORDENAÇÃO POR DURAÇÃO' as teste,
    id,
    duration,
    sdr_name,
    created_at
FROM public.get_calls_with_filters(NULL, NULL, NULL, NULL, NULL, 10, 0, 'duration')
ORDER BY duration DESC
LIMIT 10;

SELECT 'FUNÇÃO CORRIGIDA COM ORDENAÇÃO!' as status;
